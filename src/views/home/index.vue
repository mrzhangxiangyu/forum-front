<template>
  ...
  >
    <div class="dashboard-text">
      <span class="deep-tabs">
        <el-tabs v-model="activeName" @tab-click="tabClick">
          <el-tab-pane v-for="(item, index) in types" :key="'tab' + index" :label="item.name" :name="item.id.toString()" />
        </el-tabs>
      </span>
      ...
      return new Promise((resolve, reject) => {
        this.$axios({
          ...
          if (ret.data.meta.status) {
            ...
            this.activeName = this.types[0].id.toString()
            resolve()
          } else {
            this.$message.warning('获取版块信息失败')
          }
        })
      })
    },
